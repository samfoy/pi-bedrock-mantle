...
export function parsePortEnv(name, defaultPort) {
    ...
    if (raw === "") {
        throw new Error(`[bedrock-mantle] Invalid ${name}=${JSON.stringify(raw)}; expected an integer port from 0 to 65535 (0 = ephemeral). Unset the variable to use the default.`);
    }
    const value = Number(raw);
    if (!Number.isInteger(value) || value < 0 || value > 65535) {
        throw new Error(`[bedrock-mantle] Invalid ${name}=${JSON.stringify(raw)}; expected an integer port from 0 to 65535 (0 = ephemeral).`);
    }
    return value;
}
/**
 ...
 * unchanged.
 */
function normalisedBody(buf, path) {
    ...
    if (!/^\/openai\/v1\/responses(\?|$|\/)/.test(path))
        return buf;
    if (buf.length === 0)
        return buf;
    let parsed;
 ...
 */
export async function signAndForward(input) {
 ...
 */
export function createSigningProxy(region, desiredPort = 0) {
 ...
 * and tests.
 */
export function startProxy(port, region) {
