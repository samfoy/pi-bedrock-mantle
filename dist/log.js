...
function formatLine(level, kind, fields) {
    const parts = [`level=${level}`, `kind=${kind}`];
    if (fields) {
...
function emit(level, kind, fields) {
    if (!shouldLog(level))
        return;
...
export function setLogFile(path) {
    ...
    if (fileSinkPath)
        ensureSinkDir(fileSinkPath);
}
function resolveFileSink() {
    if (fileSinkResolved)
        return fileSinkPath;
...
function writeToFileSink(line) {
    ...
    try {
        ...
        process.stderr.write(`[bedrock-mantle] level=warn kind=log_file_error path=${JSON.stringify(path)} error=${JSON.stringify(reason)}\n`);
    }
}
// ─── Forensic dumps ─────────────────────────────────────────────────────────
/**
 ...
 */
export function writeDump(fileName, payload) {
...
export const log = {
    error(kind, fields) { emit("error", kind, fields); },
    warn(kind, fields) { emit("warn", kind, fields); },
    info(kind, fields) { emit("info", kind, fields); },
    debug(kind, fields) { emit("debug", kind, fields); },
};
