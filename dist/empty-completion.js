 ...
 */
export function maybeDetectEmptyCompletion(upstream, ctx) {
    const noop = () => { };
    if (!isOpenAIResponsesPath(ctx.path))
        return { response: upstream, dispose: noop };
...
async function scanSseForEmptyCompletion(reader, ctx) {
...
function handleSseEvent(event, ctx) {
 ...
 */
export function inspectResponseCompleted(payload) {
    const verdict = { empty: false, outputItemTypes: [] };
    ...
    if (usage && typeof usage === "object") {
        ...
        if (typeof out === "number")
            verdict.outputTokens = out;
        const detail = usage.output_tokens_details;
